...
static const char PROGMEM WAYPOINT_HTML[] = R"rawliteral(
<html>
<head>
...
<ul id="waypoints"></ul>
<button onclick="saveWaypoints()">Save Changes</button>
<br><br>
<a href="/">Back</a>
...
function moveUp(i){ if(i>0){ [waypoints[i-1],waypoints[i]]=[waypoints[i],waypoints[i-1]]; render(); } }
function moveDown(i){ if(i<waypoints.length-1){ [waypoints[i+1],waypoints[i]]=[waypoints[i],waypoints[i+1]]; render(); } }
function del(i){ waypoints.splice(i,1); render(); }


async function saveWaypoints() {
await fetch('/update_waypoints', {
method:'POST',
...
<html>
  ...
  <body>
    <div id="mainContent">
      ...
      <button class="button" onclick="window.location.href='/waypoints_page'">Waypoint Editor</button>
    </div>
...
static esp_err_t waypoints_handler(httpd_req_t *req) {
  ...
  for (int i=0; i<recordCount; i++) {
    JsonObject obj = arr.createNestedObject();
      obj["cmd"] = recordedPath[i].cmd;
      obj["delayMs"] = recordedPath[i].delayMs;
    }
    String json;
...
static esp_err_t update_waypoints_handler(httpd_req_t *req) {
  char buf[2048];
  ...
  for (JsonObject obj : doc.as<JsonArray>()) {
    if (recordCount < MAX_RECORDS) {
      recordedPath[recordCount].cmd = (const char*)obj["cmd"];
      recordedPath[recordCount].delayMs = obj["delayMs"];
      recordCount++;
    }
  }
  return httpd_resp_sendstr(req, "Waypoints updated");
}

static esp_err_t waypoint_page_handler(httpd_req_t *req) {
  httpd_resp_set_type(req, "text/html");
  return httpd_resp_send(req, (const char*)WAYPOINT_HTML, strlen(WAYPOINT_HTML));
}
...
void startCameraServer(){
  ...
  httpd_uri_t waypoint_uri = { 
    .uri       = "/waypoints", 
    .method    = HTTP_GET, 
    .handler   = waypoints_handler, 
    .user_ctx  = NULL 
  };
  httpd_uri_t update_uri = { 
    .uri       = "/update_waypoints", 
    .method    = HTTP_POST, 
    .handler   = update_waypoints_handler, 
    .user_ctx  = NULL 
  };
  httpd_uri_t page_uri = { 
    .uri       = "/waypoints_page", 
    .method    = HTTP_GET, 
    .handler   = waypoint_page_handler, 
    .user_ctx  = NULL 
  };
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &waypoint_uri);
    httpd_register_uri_handler(camera_httpd, &update_uri);
    esp_err_t res = httpd_register_uri_handler(camera_httpd, &page_uri);
    if (res != ESP_OK) {
      Serial.printf("Failed to register /waypoints_page: %d\n", res);
    } else {
      Serial.println("/waypoints_page registered successfully");
    }
  }
